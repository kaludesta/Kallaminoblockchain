...
const AdminResources: React.FC = () => {
    ...
    const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            ...
            reader.onloadend = () => {
                setCurrentResource({ ...currentResource, url: reader.result as string });
            };
            ...
            {isModalOpen && (
                <Modal title={isEditing ? 'Edit Resource' : 'Add Resource'} onClose={() => setIsModalOpen(false)} onSave={handleSave}>
                    <div className="space-y-4">
                        ...
                        <div>
                           <label className="block text-sm font-medium mb-1">Resource File or URL</label>
                            <input name="url" value={currentResource.url.startsWith('data:') ? '' : currentResource.url} onChange={handleChange} placeholder="Or paste an external URL" className="w-full p-2 border rounded dark:bg-gray-800 dark:border-gray-600 mb-2" />
                           <input type="file" onChange={handleFileUpload} className="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 dark:file:bg-blue-900/20 dark:file:text-blue-300 dark:hover:file:bg-blue-900/30"/>
